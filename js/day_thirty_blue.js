async function drawMap_locations_blue() {
    ...
    const blue = await d3.json("./../data/blue_names_OSM.geojson")
    ...
    const blues = await d3.csv("./../data/blue_names_OSM.csv", function(d) {
        return {
            lon: +d.lon,
            lat: +d.lat
    ...
    const hexReds = hexgridReds(blues_map)

    colour = d3.scaleSequential(t => {
        return d3.interpolateBlues(Math.pow(t, 20))
