async function drawMap_topo() {
    ...
    const SA2_map_outerwest = await d3.json("./../geo/SA2_simple.json")



    SA2_map_outerwest.objects.SA2.geometries = SA2_map_outerwest.objects.SA2.geometries.filter(d => d.properties.SA4_NAME16 == "Sydney - Outer West and Blue Mountains")

    // console.log(SA2_map)


    SA2_topo_outerwest = topojson.feature(SA2_map_outerwest, SA2_map_outerwest.objects.SA2)
    ...
    trains = trains_import.filter(d=>d.SA4 != "Outer" & d.SA4=="Sydney - Outer West and Blue Mountains")
    ...
    const projection_SA2 = d3.geoEquirectangular()
        .fitWidth(dimensions.boundedWidth, SA2_topo_outerwest)
    ...
    const [
        [x0, y0],
        [x1, y1]
    ] = pathGenerator_SA2.bounds(SA2_topo_outerwest)
    ...
    const background = bounds
        ...
        .data(SA2_topo_outerwest.features)
        .join("path")
        .attr("class", "SA2")
    ...
    const points = bounds
        ...
        .attr("x", d => projection_SA2([d.longitude, d.latitude + 0.005])[0])
        .attr("y", d => projection_SA2([d.longitude, d.latitude + 0.005])[1])
