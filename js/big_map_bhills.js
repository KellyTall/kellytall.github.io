async function drawMap_topo() {
    ...
    const SA2_map_bhills = await d3.json("./../geo/SA2_simple.json")



    SA2_map_bhills.objects.SA2.geometries = SA2_map_bhills.objects.SA2.geometries.filter(d => d.properties.SA4_NAME16 == "Sydney - Baulkham Hills and Hawkesbury")

    // console.log(SA2_map)


    SA2_topo_bhills = topojson.feature(SA2_map_bhills, SA2_map_bhills.objects.SA2)
    ...
    const projection_SA2 = d3.geoEquirectangular()
        .fitWidth(dimensions.boundedWidth, SA2_topo_bhills)
    ...
    const [
        [x0, y0],
        [x1, y1]
    ] = pathGenerator_SA2.bounds(SA2_topo_bhills)
    ...
    const wrapper = d3.select(".map_bhills")
        .append("svg")
