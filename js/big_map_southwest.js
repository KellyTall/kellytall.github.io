async function drawMap_topo() {
    ...
    const SA2_map_southwest = await d3.json("./../geo/SA2_simple.json")



    SA2_map_southwest.objects.SA2.geometries = SA2_map_southwest.objects.SA2.geometries.filter(d => d.properties.SA4_NAME16 == "Sydney - South West")

    // console.log(SA2_map)


    SA2_topo_southwest = topojson.feature(SA2_map_southwest, SA2_map_southwest.objects.SA2)
    ...
    trains = trains_import.filter(d=>d.SA4 != "Outer" & d.SA4=="Sydney - South West")



    // console.log(trains)

    const wrapper = d3.select(".map_southwest")
       .append("svg")
    ...
    const projection_SA2 = d3.geoEquirectangular()
        .fitWidth(dimensions.boundedWidth, SA2_topo_southwest)
    ...
    const [
        [x0, y0],
        [x1, y1]
    ] = pathGenerator_SA2.bounds(SA2_topo_southwest)
    ...
    const background = bounds
        ...
        .data(SA2_topo_southwest.features)
        .join("path")
        .attr("class", "SA2")
