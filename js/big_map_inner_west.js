async function drawMap_topo() {
    ...
    const SA2_map_inner_west = await d3.json("./../geo/SA2_simple.json")



    SA2_map_inner_west.objects.SA2.geometries = SA2_map_inner_west.objects.SA2.geometries.filter(d => d.properties.SA4_NAME16 == "Sydney - Inner West")

    // console.log(SA2_map_inner_west)


    SA2_topo_inner_west = topojson.feature(SA2_map_inner_west, SA2_map_inner_west.objects.SA2)
    ...
    trains = trains_import.filter(d=>d.SA4 != "Outer" & d.SA4=="Sydney - Inner West")

    // console.log(trains)

   const wrapper = d3.select(".map_inner_west")
        .append("svg")
    ...
    const projection_SA2 = d3.geoEquirectangular()
        .fitWidth(dimensions.boundedWidth, SA2_topo_inner_west)
    ...
    const [
        [x0, y0],
        [x1, y1]
    ] = pathGenerator_SA2.bounds(SA2_topo_inner_west)
    ...
    const background = bounds
        ...
        .data(SA2_topo_inner_west.features)
        .join("path")
        .attr("class", "SA2")
