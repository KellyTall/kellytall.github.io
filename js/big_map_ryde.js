async function drawMap_topo() {
    ...
    const SA2_map_ryde = await d3.json("./../geo/SA2_simple.json")



    SA2_map_ryde.objects.SA2.geometries = SA2_map_ryde.objects.SA2.geometries.filter(d => d.properties.SA4_NAME16 == "Sydney - Ryde")

    // console.log(SA2_map_ryde)


    SA2_topo_ryde = topojson.feature(SA2_map_ryde, SA2_map_ryde.objects.SA2)

    // console.log(SA2_topo_ryde)
    ...
    const projection_SA2 = d3.geoEquirectangular()
        .fitWidth(dimensions.boundedWidth, SA2_topo_ryde)
    ...
    const [
        [x0, y0],
        [x1, y1]
    ] = pathGenerator_SA2.bounds(SA2_topo_ryde)
    ...
    const points = bounds
        ...
        .attr("y", d => projection_SA2([d.longitude, d.latitude + 0.00])[1])
