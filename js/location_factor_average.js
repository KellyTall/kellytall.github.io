async function drawMap_locations() {
    ...
    location_filter = location.filter(d => d.type =="average")
    ...
    const svg = d3.select(".location_factor_holder_average")
        .append("svg")
    ...
    const location_mapped = new Map(location_filter.map(d => [d.SA4_NAME16, d.factor]))

    const max_factor = d3.max(location_filter, d => d.factor)
    const min_factor = d3.min(location_filter, d => d.factor)
