async function drawstatus() { 
    ...
    const data_all = await d3.csv("./../data/travel_mode.csv", function(d) {

        return {
            mode_travel: d.mode_travel,
    ...
    const data_unsort = data_all.filter(d => d.sa4 == "Parramatta")
    ...
    const names = Array.from(new Set(data.map(d => d.mode_travel)))
...
const width = 380
    ...
    const svg = d3.select(".parramatta_transport")
        .append("svg")
        .data(data)
        .attr("width", width)
        .attr("height", height)
...
const yScale = d3.scaleBand()
    ...
    const yAxis = d3.axisLeft()
        .scale(yScale)
        .tickFormat(i => data[i].mode_travel)
    .tickSizeOuter(0)
    .tickSizeInner(2)
