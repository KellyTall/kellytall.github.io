async function drawMap_locations() {
    ...
    const red = await d3.json("./../data/red_names_OSM.geojson")
    ...
    const reds = await d3.csv("./../data/red_names_OSM.csv", function(d) {
        return {
            lon: +d.lon,
            lat: +d.lat
    ...
    const hexReds = hexgridReds(reds_map)

    colour = d3.scaleSequential(t => {
        return d3.interpolateReds(Math.pow(t, 20))
