async function drawMap_topo() {


    const SA2_map_Sutherland = await d3.json("./../geo/SA2_simple.json")



    SA2_map_Sutherland.objects.SA2.geometries = SA2_map_Sutherland.objects.SA2.geometries.filter(d => d.properties.SA4_NAME16 == "Sydney - Sutherland")

    console.log(SA2_map_Sutherland)


    SA2_topo_Sutherland = topojson.feature(SA2_map_Sutherland, SA2_map_Sutherland.objects.SA2)

    console.log(SA2_topo_Sutherland)

    const trains_import = await d3.csv("./../data/tfnsw_train.csv", function(d) {
        return {
            location_name: d.location_name,
            latitude: +d.latitude,
            longitude: +d.longitude,
            SA4: d.SA4

        }
    })

    trains = trains_import.filter(d => d.SA4 != "Outer" & d.SA4 == "Sydney - Sutherland")

    // console.log(trains)

    

 const width = 1000
    const height = 1000
    const margin = { top: 40, right: 50, bottom: 10, left: 100 }



    const svg_big_map = d3.select(".map_Sutherland")
        .append("svg")
        .attr("viewBox", "0 0 1000 1000")
        .attr('transform', `translate(0,${margin.top})`)



    
    const projection_SA2 = d3.geoEquirectangular()
        .fitWidth(width, SA2_topo_Sutherland)



    const pathGenerator_SA2 = d3.geoPath(projection_SA2)


function wrap(text, width) {
        text.each(function() {
            var text = d3.select(this),
                words = text.text().split(/\s+/).reverse(),
                word,
                line = [],
                lineNumber = 0,
                lineHeight = 1.1, // ems
                x = text.attr("x"),
                y = text.attr("y"),
                dy = 0, //parseFloat(text.attr("dy")),
                tspan = text.text(null)
                .append("tspan")
                .attr("x", x)
                .attr("y", y)
                .attr("dy", dy + "em");
            while (word = words.pop()) {
                line.push(word);
                tspan.text(line.join(" "));
                if (tspan.node().getComputedTextLength() > width) {
                    line.pop();
                    tspan.text(line.join(" "));
                    line = [word];
                    tspan = text.append("tspan")
                        .attr("x", x)
                        .attr("y", y)
                        .attr("dy", ++lineNumber * lineHeight + dy + "em")
                        .text(word);
                }
            }
        });
    }


   

    const background = svg_big_map
    .append("g")
        .selectAll(".background")
        .data(SA2_topo_Sutherland.features)
        .join("path")
        .attr("class", "SA4_big_map")
        .attr("d", pathGenerator_SA2)


    const labels = svg_big_map
    .append("g")
        .selectAll("text")
        .data(SA2_topo_Sutherland.features)
        .join("text")
        .attr("class", "area_label_extra_small")
        .text(d => d.properties.SA2_NAME16)
        .attr("x", d => pathGenerator_SA2.centroid(d)[0])
        .attr("y", d => pathGenerator_SA2.centroid(d)[1])
        .call(wrap, 80)

    const points = svg_big_map
    .append("g")
        .selectAll("circle")    
        .data(trains)    
        .join("circle")
        .attr("class", "circle")
        .attr("cx", d => projection_SA2([d.longitude, d.latitude])[0])
        .attr("cy", d => projection_SA2([d.longitude, d.latitude])[1])
        .attr("r", 4)

        const point_label = svg_big_map
        .append("g")
        .selectAll("text")    
        .data(trains)    
        .join("text")
        .attr("class", "point_label")
        .text(d => d.location_name)
        .attr("x", d => projection_SA2([d.longitude, d.latitude + 0.007])[0])
        .attr("y", d => projection_SA2([d.longitude, d.latitude + 0.007])[1])
        .call(wrap, 60)
        
    



}

drawMap_topo()